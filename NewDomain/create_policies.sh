...
echo "Log file: $LOGFILE"
      ...
      "statements": [
        ...
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage all-resources in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage dns in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage cluster-node-pools in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage cluster-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage file-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage compute-management-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage instance-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage volume-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use ons-topics in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use subnets in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use virtual-network-family in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use vnics in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use network-security-groups in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to inspect compartments in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to {CLUSTER_JOIN} in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to read metrics in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use metrics in compartment $COMPARTMENT_NAME where target.metrics.namespace='gpu_infrastructure_health'",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to use metrics in compartment $COMPARTMENT_NAME where target.metrics.namespace='rdma_infrastructure_health'",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to read repos in compartment $COMPARTMENT_NAME",
        "Allow dynamic-group $AI_DOMAIN_NAME/'oci-hpc-dg' to manage repos in compartment $COMPARTMENT_NAME"
      ]
      ...
      "statements": [
        ...
        "allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage all-resources in compartment $COMPARTMENT_NAME",
        "allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage dns in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage orm-family in compartment $COMPARTMENT_NAME",
        ...
        "Allow any-user to manage all-resources in compartment $COMPARTMENT_NAME where all { request.principal.type = 'workload', request.principal.service_account = 'oke-aznz-svcacct', request.principal.cluster_id = 'ocid1.cluster.oc1.us-dallas-1.' }",
        "Allow any-user to manage all-resources in compartment $COMPARTMENT_NAME where all { request.principal.type = 'workload', request.principal.service_account = 'cluster-autoscaler', request.principal.cluster_id = 'ocid1.cluster.oc1.us-dallas-1.' }",
        "Allow any-user to manage file-family in compartment $COMPARTMENT_NAME where request.principal.type = 'cluster'",
        "Allow any-user to use virtual-network-family in compartment $COMPARTMENT_NAME where request.principal.type = 'cluster'",
        ...
        "Allow group  $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage buckets in compartment $COMPARTMENT_NAME",
        "Allow group  $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage objects in compartment $COMPARTMENT_NAME",
        "Allow group  $AI_DOMAIN_NAME/$AI_GROUP_NAME to read repos in compartment $COMPARTMENT_NAME",
        "Allow group  $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage repos in compartment $COMPARTMENT_NAME"
      ]
      ...
      "statements": [
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage instance-family in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use subnets in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage virtual-network-family in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to inspect compartments in tenancy",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use vnics in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use network-security-groups  in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use private-ips  in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage public-ips  in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage volume-family in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage cluster-family in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage vcns in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage subnets in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage internet-gateways in  compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage nat-gateways in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage route-tables in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage security-lists in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to inspect clusters in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use cluster-node-pools in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to read cluster-work-requests in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to manage service-gateways in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use cloud-shell in  compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to read vaults in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to read keys in compartment $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to use compute-capacity-reservations in compartment  $COMPARTMENT_NAME",
        "Allow group $AI_DOMAIN_NAME/$AI_GROUP_NAME to read metrics in compartment $COMPARTMENT_NAME"
      ]
